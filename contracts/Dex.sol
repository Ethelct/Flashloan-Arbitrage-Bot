...
contract Dex {
    ...
    function buyDAI() external {
        ...
        require(dai.balanceOf(address(this)) >= daiToReceive, "Not enough DAI liquidity");

        // Transfer DAI and update USDC balance
        dai.transfer(msg.sender, daiToReceive);
        usdcBalances[msg.sender] = 0; // Reset USDC balance to prevent reuse
    }

    function sellDAI() external {
        uint256 usdcToReceive = ((daiBalances[msg.sender] * dexBRate) / 100) / (10**12);
        require(usdc.balanceOf(address(this)) >= usdcToReceive, "Not enough USDC liquidity");

        // Transfer USDC and update DAI balance
        usdc.transfer(msg.sender, usdcToReceive);
        daiBalances[msg.sender] = 0; // Reset DAI balance to prevent reuse
    }
